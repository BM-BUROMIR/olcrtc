...
func RunWithReady(ctx context.Context, cfg Config, onReady func()) error {
	...
	socksPolicy, err := newSOCKSBlockPolicy(cfg.SOCKSBlockPolicy)
	if err != nil {
...
func (c *Client) handleSocks5(ctx context.Context, conn net.Conn) {
	...
	if c.socksPolicy.blocks(targetAddr, targetPort) {
		...
		_, _ = conn.Write(replyConnectionNotAllowed())
		return
	}
...
func replyConnectionNotAllowed() []byte {
	return []byte{5, 2, 0, 1, 0, 0, 0, 0, 0, 0}
}
