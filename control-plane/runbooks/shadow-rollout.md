...
- Provision mode-0600 credentials: `rotation.env`, `ssh_key`, `telemost.cookies`,
  `deployment.json`, `server-base.yaml`, and pinned `known_hosts`.
...
sudo systemctl enable --now olc-control-plane-shadow.timer
...
Continue only when the legacy rotation succeeds, the matching shadow operation is `active`, no raw
provider response or credential appears in logs, and the timer survives a host reboot.
...
```bash
sudo systemctl disable --now olc-control-plane-shadow.timer
sudo systemctl stop olc-control-plane-shadow.service
```
