...
class SystemdUnitTest(unittest.TestCase):
    def test_shadow_service_is_hardened_and_uses_credentials(self) -> None:
        ...
        self.assertNotIn("managed-wb-rotation.json", service)
        self.assertNotIn("wb.bearer", service)
        self.assertNotIn("wb.room", service)
        self.assertNotIn("wb-server-base.yaml", service)
    ...
    def test_wb_has_an_independent_hardened_service_and_timer(self) -> None:
        service = (ROOT / "systemd/olc-control-plane-wb.service").read_text()
        ...
        self.assertNotIn("telemost.cookies", service)
        self.assertIn("ExecStart=/opt/olc/control-plane/run-systemd-wb-rotation.sh", service)
        self.assertIn("ExecStartPost=/opt/olc/control-plane/backup-systemd-state.sh", service)
        self.assertIn("NoNewPrivileges=yes", service)
        self.assertIn("ProtectSystem=strict", service)
        self.assertNotIn("/Users/", service)
    ...
    def test_runner_uses_systemd_credential_directory(self) -> None:
        ...
        self.assertNotIn("run-systemd-wb-rotation.sh", runner)
