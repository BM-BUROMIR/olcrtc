...
| `socks.block_ports` | Порты назначения, которые нужно отклонять до туннелирования | - |
| `socks.block_hosts` | Host-правила назначения для отклонения; поддерживаются точные имена и wildcard `*.example.com` | - |
| `socks.block_cidrs` | IP CIDR назначения, которые нужно отклонять до туннелирования | - |
...
`socks.block_*` применяется локально на SOCKS5 ingress. Отклонённые назначения не открывают tunnel streams.
